...
function getBlogPost($id) {
  ...
  fetch(strapiURL + "aw-blogposts/" + $id) //appel de l'api
    ...
    .then(
      (
        response //Donc, on refait un then
      ) => {
        deleteMainContent();
        ...
        console.log(data);
        postContainer = createDivIn(
          document.getElementById("main-content"),
          "post",
          "div",
          "blog__element blog__element--detailed-view"
        );

        postDateTag = createDivIn(
          postContainer,
          "date_tag",
          "div",
          "blog__element__date"
        );
        postImage = createDivIn(
          postContainer,
          "image",
          "img",
          "blog__element__image blog__element__image--detailed-view"
        );
        postTitle = createDivIn(
          postContainer,
          "title",
          "div",
          "blog__element__title"
        );
        postContent = createDivIn(
          postContainer,
          "text",
          "div",
          "blog__element__synopsis blog__element__synopsis--detailed-view"
        );
        ...
        postDateTag.textContent =
          data["tag"] + " / " + data["published_at"].substring(0, 10);
      }
    );
}
...
function generateAboutContent() {
  ...
  postContainer.innerHTML =
    "<img src='public/images/AW_profile.jpg'><p>Anne Waddington, auteure prolifique, partage son temps entre Toulouse et les Monts de Lacaune. </p><p> Elle aime revisiter l'Histoire et mettre en lumière, tant dans ses romans que dans ses thrillers, des éléments du passé dont les manuels évitent de parler . </p><p>Son style enlevé, la justesse de ses personnages et sa façon de distiller le suspens vous emportent jusqu'à des dénouements saisissants et inattendus. </p><p>Anciennement psychologue et thérapeute familial, la retraite lui permet de se consacrer à l'écriture et au théâtre.</p>";
}
...
function getBookList() {
  fetch(strapiURL + "aw-books?_sort=release_date:ASC") //appel de l'api
    .then(
      (
        response // Si on reçoit une réponse
      ) => {
        response
          .json() //On la transforme en json mais c'est toujours une promise j'ai pas compris pourquoi
          .then(
            (
              response //Donc, on refait un then
            ) => {
...
function generateBookItems($response) {
  ...
  booksContainer = createDivIn(
    document.getElementById("main-content"),
    "book__books-container",
    "div",
    "book__books-container"
  );
  ...
  for (i = 0; i < Object.keys($response).length; i++) {
    ...
    postImage = createDivIn(
      ...
      "image",
      "img",
      "book__element__image"
    );
    postTexts = createDivIn(
      postContainer,
      "text-container",
      "div",
      "book__element__text-container"
    );
    postTitle = createDivIn(postTexts, "title", "div", "book__element__title");
    postPublisher = createDivIn(
      postTexts,
      "publisher",
      "div",
      "book__element__publisher"
    );
    postReleaseDate = createDivIn(
      postTexts,
      "date",
      "div",
      "book__element__date"
...
function fadeContentOutIn() {
  var childrenOfMainMenu = [].slice.call(
    document.getElementById("main-content").children
  );
  childrenOfMainMenu.forEach((element) =>
    element.animate(
  ...
  document
    ...
    .animate([{ opacity: "0" }, { opacity: "0" }, { opacity: "1" }], 1);
}
          ...
          .then(
            (
              ...
              if (pageName[0].substring(0, 8) === "blogpost") {
                ...
                document
                  .getElementById(
                    "main-content__side-menu__item--" + backIcon["tagName"]
                  )
                ...
                icons.forEach((element) =>
                  createSideMenuEventListener(element)
                );
              }
            }
          );
...
function createSideMenuItem($element) {
  $currentItem = createDivIn(
    document.getElementById("main-content__side-menu"),
    "main-content__side-menu__item--" + $element["tagName"],
    "div",
    "main-content__side-menu__item main-content__side-menu__item--" +
      $element["tagName"]
  );

  $currentItem.innerHTML =
    $element["icon"] + "<p>" + $element["tagName"] + "</p>";
}
...
function createSideMenuEventListener($element) {
  document
    .getElementById("main-content__side-menu__item--" + $element["tagName"])
    .addEventListener("click", function () {
      getPostList($element["tagName"]);
    });
}
...
function createTitle($contentType, $icon) {
  sectionTitle = createDivIn(
    document.getElementById("main-content"),
    "title",
    "div",
    "main-content__title-container"
  ); // on créée le conteneur de titre
  sectionSideMEnu = createDivIn(
    document.getElementById("main-content"),
    "main-content__side-menu",
    "div",
    "main-content__side-menu"
  );
  sectionTitle.innerHTML =
    "<div id ='main-content__title-container__title' class = 'main-content__title-container__title'>" + // le titre
    $icon + //le logo
    "<h2>" +
    $contentType +
    "</h2>";
...
document.getElementById("blog-posts").onclick = function () {
  ...
  document
    ...
    .removeEventListener("click", hideMenu, false);
  document
    .getElementById("nav-expand")
